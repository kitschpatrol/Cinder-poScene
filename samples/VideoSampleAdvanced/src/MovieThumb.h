...
class MovieThumb : public po::scene::NodeContainer {
public:
	...
	void setPlayerPos(ci::vec2 pos) {
		mPlayerPos = pos;
	};
	ci::vec2 &getPlayerPos() {
		return mPlayerPos;
	};

	void setThumbnailPos(ci::vec2 pos) {
		mThumbnailPos = pos;
	};
	ci::vec2 &getThumbnailPos() {
		return mThumbnailPos;
	};

	void setPlayerScale(ci::vec2 scale) {
		mPlayerScale = scale;
	};
	ci::vec2 &getPlayerScale() {
		return mPlayerScale;
	};

	void setThumbnailScale(ci::vec2 scale) {
		mThumbnailScale = scale;
	};
	ci::vec2 &getThumbnailScale() {
	...
	po::scene::VideoGlRef getUnderlyingMovie() {
		return mMovie;
	};

	MovieThumbSignal &getSignalAnimationComplete() {
