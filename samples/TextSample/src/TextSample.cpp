...
void TextSample::setup() {
	...
	rotatingText->setAlignment(po::scene::Alignment::CENTER_CENTER).setPosition(150, 150);
	...
	mScalingText->setAlignment(po::scene::Alignment::CENTER_CENTER).setPosition(150, 350);
	...
	addChild(movingText);
	ci::app::timeline().apply(&movingText->getPositionAnim(), ci::vec2(movingText->getPosition().x, -movingText->getHeight()), 10.f).easeFn(ci::EaseInSine()).loop();
...
void TextSample::animateScale() {
	...
	ci::app::timeline().appendTo(&mScalingText->getScaleAnim(), ci::vec2(1.2f, 1.2f), 1.f).easeFn(ci::EaseInSine()).finishFn(std::bind(&TextSample::animateScale, this));
}