...
void Square::setSelected(bool isSelected) {
	...
	if (isSelected) {
		...
		ci::app::timeline().apply(&mSelected->getAlphaAnim(), 0.0f, 0.5f).finishFn(std::bind(&Square::reset, this));
	}
}

void Square::doRotate() {
	ci::app::timeline().apply(&getRotationAnim(), (float)M_PI * 2, 0.5f).finishFn(std::bind(&Square::reset, this));
}
