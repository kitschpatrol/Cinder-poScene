...
void AlignmentSample::setup() {
	...
	mShapeNode->setAlignment(po::scene::Alignment::NONE).setFillColor(122.f / 255, 201.f / 255, 67.4 / 255).setPosition(center);
	addChild(mShapeNode);

	//  create and add a reference dot to indicate position of mShapeNode
	ShapeRef dot = Shape::createCircle(10);
	dot->setAlignment(po::scene::Alignment::CENTER_CENTER).setFillColor(255.f / 255, 123.f / 255, 172.f / 255).setPosition(center);
	addChild(dot);
...
void AlignmentSample::createIndicators() {
	...
	mIndicatorNames = {"NONE", "TOP_LEFT", "TOP_CENTER", "TOP_RIGHT", "CENTER_LEFT", "CENTER_CENTER", "CENTER_RIGHT", "BOTTOM_LEFT", "BOTTOM_CENTER", "BOTTOM_RIGHT"};
