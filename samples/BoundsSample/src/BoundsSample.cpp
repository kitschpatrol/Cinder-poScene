...
void BoundsSample::setup() {
	...
	mInfoText.size(200, ci::TextBox::GROW).color(ci::Color(1, 1, 1)).alignment(ci::TextBox::Alignment::RIGHT).font(ci::Font("Arial", 11));

	// Scene BG
	mBG = Shape::createRect(1, 1);
	mBG->setFillColor(ci::Color(ci::CM_HSV, hue, saturation, brightness)).setParentShouldIgnoreInBounds(true);
	addChild(mBG);

	// Scene text box + window text box
	mWindowTextBox = TextBox::create();
	mWindowTextBox->setPosition(-250, 0).setParentShouldIgnoreInBounds(true);
	addChild(mWindowTextBox);

	mTextBox = TextBox::create();
	mTextBox->setPosition(5, 5).setParentShouldIgnoreInBounds(true);
	addChild(mTextBox);

	// Node Container
	mNodeContainer = NodeContainer::create("Node Container");
	mNodeContainer->setPosition(100, 150).setDrawBounds(true).setBoundsColor(boundsColor).setName("Node Container");

	mNodeContainerBG = Shape::createRect(1, 1);
	mNodeContainerBG->setFillColor(ci::Color(ci::CM_HSV, nodeContainerHue, saturation, brightness)).setParentShouldIgnoreInBounds(true);

	mNodeContainerTextBox = TextBox::create();

	mNodeContainerTextBox->setPosition(5, 5).setParentShouldIgnoreInBounds(true);

	mNodeContainer->addChild(mNodeContainerBG).addChild(mNodeContainerTextBox).getSignal(MouseEvent::Type::DOWN_INSIDE).connect(std::bind(&BoundsSample::nodeMouseOver, this, std::placeholders::_1));

	// Add some shapes
	int yPos = 175;
	for (int i = 0; i < 3; i++) {
		ShapeRef s = Shape::createRect(100, 100);
		s->setPosition(i * 125, yPos).setName("Selected Node").setDrawBounds(true).setBoundsColor(boundsColor).setFillColor(ci::Color(ci::CM_HSV, nodeHue, saturation, brightness)).getSignal(MouseEvent::Type::DOWN_INSIDE).connect(std::bind(&BoundsSample::nodeMouseOver, this, std::placeholders::_1));
	...
	setName("Scene (Scene Root Node)").setPosition(ci::vec2(250, 50)).setDrawBounds(true).setBoundsColor(boundsColor);
...
void BoundsSample::update() {
	// Set the BGs
	mBG->setPosition(getBounds().getUpperLeft()).setScale(getSize());

	mNodeContainerBG->setPosition(mNodeContainer->getBounds().getUpperLeft()).setScale(mNodeContainer->getSize());
	...
	mInfoText.text(ss.str()).alignment(ci::TextBox::Alignment::RIGHT);
